...
🚀 Installation
Local Development
Clone the repository bash git clone <repository-url> cd LexGuard

Create virtual environment bash python -m venv .venv source .venv/bin/activate # On Windows: .venv\Scripts\activate

Install dependencies bash pip install -r requirements.txt

Set up environment variables Create a .env file in the root directory: env GROQ_API_KEY=your_groq_api_key_here MONGODB_URI=mongodb://localhost:27017/lexguard

Start MongoDB (if running locally) bash mongod

Docker Deployment
Build the container bash docker build -t lexguard .

Run with Docker bash docker run -p 8000:8000 -p 8501:8501 -e GROQ_API_KEY=your_key lexguard

🎯 Usage
Web Application
Start the Streamlit app bash streamlit run app_agent.py Access at: http://localhost:8501

Upload a PDF contract and view real-time analysis progress

Download the generated report or export JSON data

API Usage
Start the FastAPI server bash uvicorn main:app --reload API docs at: http://localhost:8000/docs

Analyze a contract bash curl -X POST "http://localhost:8000/api/analyze" \ -H "accept: application/json" \ -H "Content-Type: multipart/form-data" \ -F "file=@contract.pdf"
...
🙏 Acknowledgments
Built with Groq for fast AI inference
PDF processing powered by PyMuPDF
UI framework: Streamlit
📞 Support
For questions or support, please open an issue on GitHub or contact the development team.

LexGuard - Making contract review intelligent, automated, and reliable.</content> <parameter name="filePath">c:\Users\VH0000805\Downloads\Hackathon\LexGuard\README.md
